...
/// https://druid.apache.org/docs/latest/ingestion/kafka-ingestion#supervisor-spec-configuration
/// https://github.com/apache/druid/blob/master/extensions-core/kafka-indexing-service/src/main/java/org/apache/druid/indexing/kafka/KafkaIndexTaskIOConfig.java
public struct KafkaIndexTaskIOConfig: Codable, Hashable, Equatable, Sendable {
    public init(
        topic: String? = nil,
        topicPattern: String? = nil,
        consumerProperties: KafkaIndexTaskConsumerProperties,
        pollTimeout: Int? = nil,
        useEarliestOffset: Bool? = nil,
        ...
        self.consumerProperties = consumerProperties
        self.pollTimeout = pollTimeout
        self.useEarliestOffset = useEarliestOffset
    ...
    /// The Kafka topic to read from. Note that once this value is established for a supervisor, updating it is not supported. To ingest data from multiple topic, use topicPattern.
    public let topic: String?

    /// Multiple Kafka topics to read from, passed as a regex pattern
    public let topicPattern: String?

    /// A map of properties to pass to the Kafka consumer.
    public let consumerProperties: KafkaIndexTaskConsumerProperties

    /// The length of time to wait for the Kafka consumer to poll records, in milliseconds.
    public let pollTimeout: Int?

    /// If a supervisor is managing a datasource for the first time, it obtains a set of starting offsets from Kafka. This flag determines whether the supervisor retrieves the earliest or latest
    /// offsets in Kafka. Under normal circumstances, subsequent tasks start from where the previous segments ended so this flag is only used on the first run.
    public let useEarliestOffset: Bool?
