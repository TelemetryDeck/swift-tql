...
struct ConvenienceAggregatorTests {
    ...
    func userCountQueryGetsPrecompiled() throws {
        let query = CustomQuery(queryType: .timeseries, dataSource: "telemetry-signals", aggregations: [.userCount(.init())])
        let precompiled = try query.precompile()
    ...
    func eventCountQueryGetsPrecompiled() throws {
        let query = CustomQuery(queryType: .timeseries, dataSource: "telemetry-signals", aggregations: [.eventCount(.init())])
        let precompiled = try query.precompile()
    ...
    func histogramQueryGetsPrecompiled() throws {
        let query = CustomQuery(queryType: .timeseries, dataSource: "telemetry-signals", aggregations: [.histogram(.init())])
        let precompiled = try query.precompile()
